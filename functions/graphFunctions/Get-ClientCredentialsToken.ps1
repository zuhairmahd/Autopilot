        ...
        try
        {
            ...
            if ($certificate)
            {
                ...
                if ($certificate.NotAfter -lt (Get-Date))
                {
                    ...
                    if ($tryBothWithFallback)
                    {
                        Write-Warning "[$functionName] Certificate expired - falling back to client secret"
                ...
                if ($certificate -and -not $certificate.HasPrivateKey)
                {
                    ...
                    if ($tryBothWithFallback)
                    {
                        Write-Warning "[$functionName] Certificate has no private key - falling back to client secret"
