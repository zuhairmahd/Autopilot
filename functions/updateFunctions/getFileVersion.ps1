function getFileVersion()
{
    ...
    if (Test-Path $executableFileName -PathType Leaf -ErrorAction SilentlyContinue)
    {
        ...
        Write-Log -LogFile $LogFile -Module $functionName -Message "Returning local version: $localVersion" -LogLevel "Verbose"
        $returnObject = @{
    ...
    else
    {
        ...
        Write-Log -LogFile $LogFile -Module $functionName -Message "Executable file '$executableFileName' not found in the current directory." -LogLevel "Error"
        return $null
    }
}
