        ...
        if (-not $Silent)
        {
            ...
            while ($true)
            {
                ...
                if ($groupChoice -eq 'include')
                {
                    ...
                    {
                        Show-EditorArrayContents -Array $currentIncludeGroups -ArrayName "groups" -FunctionName $functionName
                    }
                    else
                    {
                    ...
                    $shouldModify = Show-EditorInteractiveChoice -PromptText "`nDo you want to modify groups to include? (y/n)" -FunctionName $functionName

                    if ($shouldModify)
                    {
                        $updatedIncludeGroups = Get-GroupArrayInput -CurrentGroups $currentIncludeGroups -GroupType "include" -AccessToken $AccessToken
                        if ($null -ne $updatedIncludeGroups -and (Compare-EditorArrayContents -Array1 $currentIncludeGroups -Array2 $updatedIncludeGroups -FunctionName $functionName))
                        {
                            ...
                            $includeSuccess = Update-DomainArraySetting -SettingsFile $SettingsFile -DomainName $DomainName -SettingName "groupsToInclude" -SettingValue $updatedIncludeGroups -FunctionName $functionName
                            if ($includeSuccess)
                            {
                ...
                elseif ($groupChoice -eq 'exclude')
                {
                    ...
                    {
                        Show-EditorArrayContents -Array $currentExcludeGroups -ArrayName "groups" -FunctionName $functionName
                    }
                    else
                    {
                        Write-Host "  (no groups specified)" -ForegroundColor Gray
                    ...
                    $shouldModify = Show-EditorInteractiveChoice -PromptText "`nDo you want to modify groups to exclude? (y/n)" -FunctionName $functionName

                    if ($shouldModify)
                    {
                        $updatedExcludeGroups = Get-GroupArrayInput -CurrentGroups $currentExcludeGroups -GroupType "exclude" -AccessToken $AccessToken
                        if ($null -ne $updatedExcludeGroups -and (Compare-EditorArrayContents -Array1 $currentExcludeGroups -Array2 $updatedExcludeGroups -FunctionName $functionName))
                        {
                            ...
                            # Save changes immediately
                            Write-Host "`nSaving changes..." -ForegroundColor Yellow
                            $excludeSuccess = Update-DomainArraySetting -SettingsFile $SettingsFile -DomainName $DomainName -SettingName "groupsToExclude" -SettingValue $updatedExcludeGroups -FunctionName $functionName
                            if ($excludeSuccess)
                            {
                                Write-Host "Group settings updated successfully!" -ForegroundColor Green
                                Write-Host "`nGroup settings updated successfully. Changes will take effect immediately." -ForegroundColor Green
