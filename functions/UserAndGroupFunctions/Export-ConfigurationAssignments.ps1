function Export-ConfigurationAssignments()
{
    ...
    function Get-CorrectBaseUriForResource()
    {
        ...
        if ($odataType -match 'win32lobapp|ioslobapp|androidlobapp|iosv pp app|androidmanagedstoreapp|managediosstore|' +
            'webapp|microsoftstoreforb businessapp|wingetapp|macoslobapp|macosvppapp|macospkgapp|windowsmobilemsi|' +
    ...
    # Create a lookup dictionary for batch responses by ID for efficient matching
    # CallGraphAPI batch responses include id field (1, 2, 3...) matching request order
    # Convert IDs to strings for consistent lookup (PowerShell hashtables are type-sensitive)
    $responseLookup = @{}
    foreach ($response in $assignmentResults.value)
    {
        if ($response.id)
        {
            $responseLookup[$response.id.ToString()] = $response
        }
    }
    
    Write-Log -logFile $LogFile -module $functionName -Message "Built response lookup with $($responseLookup.Count) entries from $($assignmentResults.value.Count) batch responses" -logLevel "Verbose"
