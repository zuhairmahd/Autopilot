function Show-PagedContent()
{
    ...
    do
    {
        ...
        if (-not $Silent)
        {
            # Visual separator between pages (don't clear screen to preserve summary)
            if ($currentPage -ne 1)
            {
                Write-Host ""
                Write-Host ""
            ...
            $userInput = Read-Host "Enter 'n' for next, 'p' for previous, 'q' to quit, or a page number (1-$totalPages)"
            ...
            switch ($userInput.ToLower())
            {
                ...
                'q'
                {
                    ...
                    # Don't break here - let the check after the switch handle it
                }
                default
                {
                    ...
                    {
                        Write-Host "Invalid input. Enter 'n', 'p', 'q', or a page number" -ForegroundColor Red
                    }
                }
            }
