...
#>
    ...
    $projects = Get-ChildItem -Path "$scriptPath\src" -Filter "*.csproj" -Recurse
    ...
    if ($Clean)
    {
        ...
        $cleanedDirs = 0
        Get-ChildItem -Path "$scriptPath\src" -Include "bin", "obj" -Recurse -Directory | ForEach-Object {
...
function MergeFunctions()
{
    ...
    param (
        [Parameter(Mandatory = $true)]
        [string[]]$FilesToMerge,
        [string]$DestinationFile = "$OutputPath\merged.ps1",
...
if ([string]::IsNullOrWhiteSpace($OutputPath))
{
    ...
    $OutputFile = Join-Path -Path $PWD -ChildPath $OutputPath | Join-Path -ChildPath $exeName
}
else
{
...
else
{
    ...
    Write-Log -logFile $logFile -Message "Failed to build native DLLs. Script will run without native DLL optimizations." -module $scriptName -LogLevel 'Error'
}
