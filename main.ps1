...
if ($updateAvailable.success -eq $true -and $updateAvailable.version -gt $version.version)
{
    ...
    if ($settings.autoUpdate)
    {
        ...
        $updateResult = GetUpdates -executableFileName "$scriptPath\$scriptName" -updateURL $updateURL -SupportingFiles @($menuFile, $stringsFile) -noConfirmation
...
$mainMenu = AddMenuItem -menu $mainMenu -Name "Check for script updates" -Action {
    ...
    $updateResult = GetUpdates -executableFileName "$scriptPath\$scriptName" -updateURL $updateURL -SupportingFiles @($menuFile, $stringsFile) 
