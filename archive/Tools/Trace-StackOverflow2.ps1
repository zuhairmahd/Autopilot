...
function Install-StackMonitor
{
    ...
    # Pattern 3: $var = Get-ChildItem -Path $folder -Filter '*.ps1' -Recurse
    #            foreach ($item in $var) { . $item.FullName }
    # This is the most common pattern used in main.ps1
    # Note: The path can be either a string literal or a variable
    $assignPattern = "\`$\w+\s*=\s*Get-ChildItem\s+(?:-Path\s+)?(\`$\w+|[`"\']([^`"\']+)[`"\']).*-Filter\s+[`"\']\*\.ps1[`"\'].*-Recurse"
    $assignMatches = [regex]::Matches($content, $assignPattern, "IgnoreCase")
    foreach ($match in $assignMatches)
    {
        # For variable paths, we need to evaluate or skip
        # For now, try to find the folder dynamically
        $pathGroup = $match.Groups[1].Value
        if ($pathGroup.StartsWith('$'))
        {
            # It's a variable - look for common patterns like find-folderPath
            # or try to find the 'functions' folder relative to the script
...
- Single-line comments (lines starting with #)
    ...
    foreach ($line in $lines)
    {
        ...
        # Pattern: . "path/file.ps1" or . 'path/file.ps1'
...
function Install-StackMonitor()
{
    ...
    # Inject after each "function FunctionName
                {
                    " line
    $modifiedContent = $content -replace '(function\s+[A-Za-z0-9-_]+\s*(?:\([^\)]*\))?\s*\{)', "`$1`n$monitorCode"
