...
Describe "Function: Show-PagedContent" -Tags 'Unit', 'UtilityFunctions', 'Paging' {
    ...
    Context "Navigation simulation (mocked input)" {
        It "Should accept 'q' to quit paging" {
            # Mock Read-Host to simulate user quitting
        ...
        It "Should navigate to next page with 'n'" {
            # Mock Read-Host to simulate 'n' then 'q'
            $script:readHostCallCount = 0
            Mock Read-Host {
                $script:readHostCallCount++
                if ($script:readHostCallCount -eq 1) { return 'n' }
                else { return 'q' }
            }
            ...
            $result = Show-PagedContent -Content $content -PageSize 10
            $result | Should -Be "quit"
            $script:readHostCallCount | Should -BeGreaterThan 1
        }

        It "Should navigate to previous page with 'p'" {
            # Mock Read-Host to simulate 'n', 'p', then 'q'
            $script:readHostCallCount = 0
            Mock Read-Host {
                $script:readHostCallCount++
                if ($script:readHostCallCount -eq 1) { return 'n' }
                elseif ($script:readHostCallCount -eq 2) { return 'p' }
                else { return 'q' }
            }
            ...
            $result = Show-PagedContent -Content $content -PageSize 10
            $result | Should -Be "quit"
            $script:readHostCallCount | Should -BeGreaterThan 2
        }

        It "Should jump to specific page number" {
            # Mock Read-Host to simulate page jump then quit
            $script:readHostCallCount = 0
            Mock Read-Host {
                $script:readHostCallCount++
                if ($script:readHostCallCount -eq 1) { return '3' }
                else { return 'q' }
            }
