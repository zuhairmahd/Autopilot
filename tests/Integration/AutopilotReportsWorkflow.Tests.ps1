...
Describe "Autopilot Event Analysis Workflow" -Tags 'Integration', 'Reports' {
    ...
    Context "Per-user date range optimization" {
        ...
        It "Should use per-user date ranges when fetching sign-in logs" {
            ...
            $analysis = Get-AutopilotEventAnalysis -AccessToken "test-token" -ApplyLocationAnalysis
    ...
    Context "ApplyLocationAnalysis switch" {
        ...
        It "Should skip sign-in correlation by default" {
            # Track Graph API calls
            $script:GraphAPICalls = @()

            Mock CallGraphAPI {
                param($ResourcePath, $AccessToken, $ExtraParameters)
                # Use comma operator to preserve arrays as single elements
                $script:GraphAPICalls += , $ResourcePath
            ...
            # Call WITHOUT ApplyLocationAnalysis
            $analysis = Get-AutopilotEventAnalysis -AccessToken "test-token"

            # Should not have called sign-in APIs
            $signInCalls = $script:GraphAPICalls | Where-Object { $_ -match 'auditLogs/signIns' }
            $signInCalls | Should -BeNullOrEmpty

            # Should have default empty location analysis
            # LocationAnalysis should be an array (empty when switch not used)
            @($analysis.LocationAnalysis) -is [array] | Should -Be $true
            $analysis.LocationAnalysisCount | Should -Be 0
        }
        ...
        It "Should perform sign-in correlation when ApplyLocationAnalysis is present" {
            ...
            # Call WITH ApplyLocationAnalysis
            $analysis = Get-AutopilotEventAnalysis -AccessToken "test-token" -ApplyLocationAnalysis

            # Should have called sign-in APIs (batch requests with sign-ins)
            $batchCalls = $script:GraphAPICalls | Where-Object { $_ -is [array] }
            $batchCalls | Should -Not -BeNullOrEmpty
        }

        It "Should display location analysis only when available" {
            Mock Write-Host { }

            # Without location analysis
            Mock CallGraphAPI {
                param($ResourcePath)
                if ($ResourcePath -eq 'deviceManagement/autopilotEvents')
                {
                    return @{ value = @($script:SampleAutopilotEvents[0]) }
                }
                if ($ResourcePath -match '^deviceManagement/managedDevices/')
                {
                    return @{
            ...
            $analysisWithoutLocation = Get-AutopilotEventAnalysis -AccessToken "test-token"

            # Should not throw when showing analysis without location data
            {
                Show-AutopilotEventAnalysis -AnalysisData $analysisWithoutLocation -ShowSummary
            } | Should -Not -Throw

            # Should not throw even if ShowLocationAnalysis switch is used when data is not available
            {
                Show-AutopilotEventAnalysis -AnalysisData $analysisWithoutLocation -ShowSummary -ShowLocationAnalysis
            } | Should -Not -Throw
        }
    }
}
