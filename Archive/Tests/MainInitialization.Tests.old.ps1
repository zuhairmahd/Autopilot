...
BeforeAll {
    ...
    # Store original location and working directory
    ...
    # Create test configuration files in the test root
    ...
    $script:InvokeMainScript = {
        ...
        # Change to repo root for execution (main.ps1 expects to run from repo root)
        Push-Location $script:RepoRoot
        try {
            # Execute main.ps1 and capture output
            $output = & $script:MainScriptPath @Parameters 2>&1
...
Describe "Main Initialization - Application Startup" -Tags 'Integration', 'Main', 'Initialization' {
    ...
    Context "Temporary File Cleanup" {
        ...
        It "Should clean up temporary files on startup" {
            # Create a mock Remove-TempFiles result
            $mockResult = @{
                AllRemoved        = $true
                RemovedFilesCount = 5
                FailedFilesCount  = 0
