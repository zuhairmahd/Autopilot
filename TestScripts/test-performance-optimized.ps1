...
try
{
    ...
    function Write-Log
    {
        ...
        # Mock for testing - silent operation
    }
    ...
    # Test 3: Application Startup Performance (End-to-End)
    Write-Host ""
    Write-Host "Test 3: Application Startup Performance" -ForegroundColor Cyan
        ...
        foreach ($scenario in $scenarios)
        {
            ...
            for ($i = 1; $i -le $Iterations; $i++)
            {
                ...
                try
                {
                    ...
                    # Using -testMode to prevent interactive prompts and menu navigation
                    $scriptBlock = @"
try {
...
"@
                    ...
                    $tempScript = Join-Path $TestTempFolder "startup-test-$i.ps1"
                    $scriptBlock | Out-File -FilePath $tempScript -Encoding UTF8
                    
                    $process = Start-Process -FilePath "pwsh" -ArgumentList "-NoProfile", "-File", $tempScript -Wait -PassThru -NoNewWindow -RedirectStandardOutput (Join-Path $TestTempFolder "stdout-$i.txt") -RedirectStandardError (Join-Path $TestTempFolder "stderr-$i.txt")
                    ...
                    {
                        Write-Host "    Iteration $i failed with exit code $($process.ExitCode)" -ForegroundColor Yellow
                        if ($DetailedOutput)
                        {
                            $stderr = Get-Content (Join-Path $TestTempFolder "stderr-$i.txt") -Raw -ErrorAction SilentlyContinue
            ...
            {
                Write-Host "  ⚠ All iterations failed for $($scenario.Description)" -ForegroundColor Yellow
            }
        }
    ...
    Write-Host ""
    Write-Host "Test 4: Memory Usage Validation" -ForegroundColor Cyan
