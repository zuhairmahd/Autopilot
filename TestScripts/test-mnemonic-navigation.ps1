...
try {
    ...
    Write-Host "[PASS] DisplayNumericMenu function exists" -ForegroundColor Green
    ...
    Write-Host "[PASS] Testing menu with Back option available" -ForegroundColor Green
    
    # Test menu with Main Menu option
    $choicesWithMainMenu = @("Option 1", "Option 2", "Back", "Main Menu")
    Write-Host "[PASS] Testing menu with Main Menu option available" -ForegroundColor Green
    
    # Test menu with no navigation options
    $choicesNoNav = @("Option 1", "Option 2")
    Write-Host "[PASS] Testing menu with no navigation options" -ForegroundColor Green
    ...
    Write-Host "[PASS] Back option correctly detected" -ForegroundColor Green
    ...
    Write-Host "[PASS] Main Menu option correctly detected" -ForegroundColor Green
    ...
    Write-Host "[PASS] Exit mnemonics 'q' and 'e' are universally available" -ForegroundColor Green
    ...
    Write-Host "[PASS] Test menu navigation context established" -ForegroundColor Green
    ...
    Write-Host "[PASS] Back navigation expected: $shouldHaveBack" -ForegroundColor Green
    Write-Host "[PASS] Main Menu navigation expected: $shouldHaveMainMenu" -ForegroundColor $(if ($shouldHaveMainMenu) { "Green" } else { "Yellow" })
    ...
    $numericKeys = @("0", "1", "2", "3")
    Write-Host "[PASS] Numeric navigation keys remain valid: $($numericKeys -join ', ')" -ForegroundColor Green
    
    # Ensure existing navigation strings still work
    $navOptions = @("Back", "Main Menu", "Exit")
    Write-Host "[PASS] Existing navigation options remain valid: $($navOptions -join ', ')" -ForegroundColor Green
    
    Write-Host ""
    Write-Host "[PASS] All mnemonic navigation tests completed successfully!" -ForegroundColor Green
    Write-Host ""
    Write-Host "Implementation verified:" -ForegroundColor Cyan
    Write-Host "  [PASS] Mnemonic key 'm' maps to Main Menu navigation" -ForegroundColor Green
    Write-Host "  [PASS] Mnemonic key 'b' maps to Back navigation" -ForegroundColor Green  
    Write-Host "  [PASS] Mnemonic keys 'q' and 'e' map to Exit application" -ForegroundColor Green
    Write-Host "  [PASS] Numeric keys (1, 2, 3, etc.) continue to work" -ForegroundColor Green
    Write-Host "  [PASS] All existing navigation remains unchanged" -ForegroundColor Green
    Write-Host "  [PASS] Easter egg functionality (not documented in UI)" -ForegroundColor Green
    Write-Host "  [PASS] No breaking changes to existing behavior" -ForegroundColor Green
