...
# Load the function
Write-Host "`n1. Loading GetAppAssignmentTypes function..." -ForegroundColor Cyan
try {
    . ".\functions\GetAppAssignmentTypes.ps1"
    Write-Host "✓ Function loaded successfully" -ForegroundColor Green
} catch {
    Write-Host "✗ Error loading function: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
if ($accessTokenParam.Attributes.Mandatory) {
    Write-Host "✓ AccessToken parameter is mandatory" -ForegroundColor Green
} else {
    Write-Host "✗ AccessToken parameter should be mandatory" -ForegroundColor Red
}

# Test Export parameter (switch)
$exportParam = $parameters['Export']
if ($exportParam -and $exportParam.ParameterType -eq [switch]) {
    Write-Host "✓ Export parameter is a switch" -ForegroundColor Green
} else {
    Write-Host "✗ Export parameter should be a switch" -ForegroundColor Red
}
...
if ($exportParameterSet) {
    Write-Host "✓ outputPath parameter is mandatory in Export parameter set" -ForegroundColor Green
} else {
    Write-Host "✗ outputPath parameter should be mandatory when Export is used" -ForegroundColor Red
}
...
if ($validateSet -and $validateSet.ValidValues -contains 'Append' -and $validateSet.ValidValues -contains 'Overwrite') {
    Write-Host "✓ fileMode parameter has correct validation: $($validateSet.ValidValues -join ', ')" -ForegroundColor Green
} else {
    Write-Host "✗ fileMode parameter should validate Append and Overwrite" -ForegroundColor Red
}
...
if ($functionContent -match '\[PSCustomObject\]') {
    Write-Host "✓ Function uses [PSCustomObject] for robust CSV output" -ForegroundColor Green
} else {
    Write-Host "✗ Function should use [PSCustomObject] for CSV output" -ForegroundColor Red
}
...
if ($verboseCount -gt 30 -and $logCount -gt 30) {
    Write-Host "✓ Function has extensive logging: $verboseCount Write-Verbose + $logCount Write-Log statements" -ForegroundColor Green
} else {
...
if ($functionContent -match 'Intent.*=.*"Required"' -and $functionContent -match 'Intent.*=.*"Available"' -and $functionContent -match 'Intent.*=.*"Unassigned"') {
    Write-Host "✓ Function adds Intent/category column to CSV export" -ForegroundColor Green
} else {
    Write-Host "✗ Function should add Intent column with Required/Available/Unassigned values" -ForegroundColor Red
}
...
if ($functionContent -match 'RequiredApps.*=' -and $functionContent -match 'AvailableApps.*=' -and $functionContent -match 'UnassignedApps.*=' -and $functionContent -match 'AllApps.*=') {
    Write-Host "✓ Function creates structured return object with all required properties" -ForegroundColor Green
} else {
    Write-Host "✗ Function should return object with RequiredApps, AvailableApps, UnassignedApps, AllApps" -ForegroundColor Red
}
...
if ($functionContent -match 'return \$returnedApps' -and $functionContent -notmatch 'return \$exportSuccessful, \$returnedApps') {
    Write-Host "✓ Function returns structured object (not tuple)" -ForegroundColor Green
} else {
    Write-Host "✗ Function should return structured object, not tuple" -ForegroundColor Red
}

# Test for CSV export handling
if ($functionContent -match 'if \(\$Export\)' -and $functionContent -match 'Export-Csv') {
    Write-Host "✓ Function handles CSV export correctly" -ForegroundColor Green
} else {
    Write-Host "✗ Function should handle CSV export when Export parameter is used" -ForegroundColor Red
}

# Test for file mode handling
if ($functionContent -match "fileMode -eq 'Append'" -and $functionContent -match "-Append") {
    Write-Host "✓ Function handles Append mode correctly" -ForegroundColor Green
} else {
    Write-Host "✗ Function should handle Append file mode" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✓ All usage examples from problem statement have valid syntax" -ForegroundColor Green
} catch {
    Write-Host "✗ Usage example syntax error: $($_.Exception.Message)" -ForegroundColor Red
}
